...
-----------------------
function worldboss_fiveScreenplay:spawnMobiles(spawnList)
	for k,v in ipairs(spawnList) do 
		...
		creature:setScreenPlayState(k, "worldboss_fiveScreenplay")
...
function worldboss_fiveScreenplay:bossDead(pBoss)
	...
	local creature = CreatureObject(pBoss)
	local listNum = creature:getScreenPlayState("worldboss_fiveScreenplay")
	local v = self.spawnPoints[listNum]
	createEvent(120 * 1000, "worldboss_fiveScreenplay", "KillBoss", pBoss, "")
	local pBoss = spawnMobile(self.planet, "WORLDBOSS", -1, math.random(v.radius) + v.x, 0, math.random(v.radius) + v.y, math.random(360), 0)
	local creature = CreatureObject(pBoss)
	print("Respawning World Boss One")
	creature:setScreenPlayState(listNum, "worldboss_fiveScreenplay")
	createObserver(OBJECTDESTRUCTION, "worldboss_fiveScreenplay", "bossDead", pBoss)
	return 0
end
-----------------------------------------------------------------------------
