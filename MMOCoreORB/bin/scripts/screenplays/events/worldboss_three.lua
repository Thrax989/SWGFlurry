...
-----------------------
function worldboss_threeScreenplay:spawnMobiles(spawnList)
	for k,v in ipairs(spawnList) do 
		...
		creature:setScreenPlayState(k, "worldboss_threeScreenplay")
...
function worldboss_threeScreenplay:bossDead(pBoss)
	...
	local creature = CreatureObject(pBoss)
	local listNum = creature:getScreenPlayState("worldboss_threeScreenplay")
	local v = self.spawnPoints[listNum]
	createEvent(120 * 1000, "worldboss_threeScreenplay", "KillBoss", pBoss, "")
	local pBoss = spawnMobile(self.planet, "WORLDBOSS", -1, math.random(v.radius) + v.x, 0, math.random(v.radius) + v.y, math.random(360), 0)
	local creature = CreatureObject(pBoss)
	print("Respawning World Boss One")
	creature:setScreenPlayState(listNum, "worldboss_threeScreenplay")
	createObserver(OBJECTDESTRUCTION, "worldboss_threeScreenplay", "bossDead", pBoss)
	return 0
end
-----------------------------------------------------------------------------
