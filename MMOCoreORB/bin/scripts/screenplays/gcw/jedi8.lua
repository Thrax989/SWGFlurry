...
registerScreenPlay("jedi8", true)
  
function jedi8:start()
    	self:spawnActiveAreas()
end
  
function jedi8:spawnActiveAreas()
	...
	if (pSpawnArea ~= nil) then
		local activeArea = LuaActiveArea(pSpawnArea)
	        ...
	        createObserver(ENTEREDAREA, "jedi8", "notifySpawnArea", pSpawnArea)
	        createObserver(EXITEDAREA, "jedi8", "notifySpawnAreaLeave", pSpawnArea)
	    end
end
 
function jedi8:notifySpawnArea(pActiveArea, pMovingObject)
...
function jedi8:notifySpawnAreaLeave(pActiveArea, pMovingObject)
