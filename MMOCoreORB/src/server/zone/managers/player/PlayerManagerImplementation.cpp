...
//BH Rank Custom Automated Progression System
	if (xpType == "bh_rank_xp") {
		if (player->hasSkill("bh_guild_system_novice")) {
			PlayerObject* ghost = player->getPlayerObject();
			const SkillList* skillList = player->getSkillList();
			int curExp = ghost->getExperience("bh_guild_xp");
			if (curExp < -15000) {
				if (player->hasSkill("bh_guild_system_novice")) {
					while (player->hasSkill("force_guild_light_novice")) {
						for (int i = 0; i < skillList->size(); ++i) {
							Skill* skill = skillList->get(i);
							if (skill->getSkillName().indexOf("bh_guild_") != -1 && skill->getSkillName().indexOf("bh_guild_system_novice") == -1) {
								SkillManager::instance()->surrenderSkill(skill->getSkillName(), player, true);
							}
						}
					}
					if (player->getScreenPlayState("bh_FRS") == 4) {
						player->setScreenPlayState("bh_FRS", 16);
					}
					String playerName = player->getFirstName();
					StringBuffer zBroadcast;
					zBroadcast << "\\#ffb90f" << playerName << " has left the \\#22b7f6BH Guild!";
					ghost->getZoneServer()->getChatManager()->broadcastGalaxy(nullptr, zBroadcast.toString());
				} 
			//error("bhSkillCheck Current FRSXP = " + String::valueOf(curExp));
			if (curExp < 10000) {
...
//FRS Custom Automated Progression System
