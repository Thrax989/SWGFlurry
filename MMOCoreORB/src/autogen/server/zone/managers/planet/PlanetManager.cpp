/*
 *	autogen/server/zone/managers/planet/PlanetManager.cpp generated by engine3 IDL compiler 0.60
 */
...
bool PlanetManager::isBuildingPermittedAt(float x, float y, SceneObject* objectTryingToBuild, float margin) {
	PlanetManagerImplementation* _implementation = static_cast<PlanetManagerImplementation*>(_getImplementation());
	if (unlikely(_implementation == NULL)) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_ISBUILDINGPERMITTEDAT__FLOAT_FLOAT_SCENEOBJECT_FLOAT_);
		method.addFloatParameter(x);
		method.addFloatParameter(y);
		...
		return _implementation->isBuildingPermittedAt(x, y, objectTryingToBuild, margin);
	}
}
...
PlanetManagerImplementation::PlanetManagerImplementation(Zone* planet, ZoneProcessServer* srv) {
	...
	terrainManager = _ref0 = new TerrainManager();
...
void PlanetManagerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_ISSPAWNINGPERMITTEDAT__FLOAT_FLOAT_FLOAT_:
		...
		break;
	case RPC_ISBUILDINGPERMITTEDAT__FLOAT_FLOAT_SCENEOBJECT_FLOAT_:
		{
			float x = inv->getFloatParameter();
			float y = inv->getFloatParameter();
			...
			bool _m_res = isBuildingPermittedAt(x, y, objectTryingToBuild, margin);
			resp->insertBoolean(_m_res);
		}
		break;
	...
	case RPC_ISINOBJECTSNOBUILDZONE__FLOAT_FLOAT_FLOAT_:
		{
			...
			bool _m_res = isInObjectsNoBuildZone(x, y, extraMargin);
			resp->insertBoolean(_m_res);
		}
		break;
...
bool PlanetManagerAdapter::isBuildingPermittedAt(float x, float y, SceneObject* objectTryingToBuild, float margin) {
	return (static_cast<PlanetManager*>(stub))->isBuildingPermittedAt(x, y, objectTryingToBuild, margin);
}
