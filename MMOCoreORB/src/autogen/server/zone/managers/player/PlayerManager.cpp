...
void PlayerManager::disseminateExperience(TangibleObject* destructedObject, ThreatMap* threatMap, SynchronizedVector<ManagedReference<CreatureObject* > >* lairSpawnedCreatures) {
	PlayerManagerImplementation* _implementation = static_cast<PlayerManagerImplementation*>(_getImplementation());
	if (unlikely(_implementation == NULL)) {
		throw ObjectNotLocalException(this);

	} else {
		_implementation->disseminateExperience(destructedObject, threatMap, lairSpawnedCreatures);
	}
}
	...
	case RPC_AWARDEXPERIENCE__CREATUREOBJECT_STRING_INT_BOOL_FLOAT_:
		{
			...
			awardExperience(player, xpType, amount, sendSystemMessage, localMultiplier);
	...
	case RPC_HEALENHANCE__CREATUREOBJECT_CREATUREOBJECT_BYTE_INT_FLOAT_:
		{
			...
			int _m_res = healEnhance(enhancer, patient, attribute, buffvalue, duration);
			resp->insertSignedInt(_m_res);
		}
		break;
