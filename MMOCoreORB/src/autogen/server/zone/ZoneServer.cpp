/*
 *	autogen/server/zone/ZoneServer.cpp generated by engine3 IDL compiler 0.60
 */
...
void ZoneServer::setShouldDeleteNavRegions(bool b) {
	ZoneServerImplementation* _implementation = static_cast<ZoneServerImplementation*>(_getImplementation());
	if (unlikely(_implementation == NULL)) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_SETSHOULDDELETENAVREGIONS__BOOL_);
		method.addBooleanParameter(b);

		method.executeWithVoidReturn();
	} else {
		_implementation->setShouldDeleteNavRegions(b);
	}
}

bool ZoneServer::shouldDeleteNavRegions() {
	ZoneServerImplementation* _implementation = static_cast<ZoneServerImplementation*>(_getImplementationForRead());
	if (unlikely(_implementation == NULL)) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_SHOULDDELETENAVREGIONS__);

		return method.executeWithBooleanReturn();
	} else {
		return _implementation->shouldDeleteNavRegions();
	}
}
...
bool ZoneServerImplementation::isServerShuttingDown() {
	// server/zone/ZoneServer.idl():  		return serverState == SHUTTINGDOWN;
...
void ZoneServerImplementation::setShouldDeleteNavRegions(bool b) {
	// server/zone/ZoneServer.idl():  		deleteNavRegions = b;
	deleteNavRegions = b;
}

bool ZoneServerImplementation::shouldDeleteNavRegions() {
	// server/zone/ZoneServer.idl():  		return deleteNavRegions;
	return deleteNavRegions;
}
...
void ZoneServerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_SETSERVERSTATE__INT_:
		...
		break;
	case RPC_SETSHOULDDELETENAVREGIONS__BOOL_:
		{
			bool b = inv->getBooleanParameter();
			
			setShouldDeleteNavRegions(b);
			
		}
		break;
	case RPC_SHOULDDELETENAVREGIONS__:
		{
			
			bool _m_res = shouldDeleteNavRegions();
			resp->insertBoolean(_m_res);
		}
		break;
...
void ZoneServerAdapter::setShouldDeleteNavRegions(bool b) {
	(static_cast<ZoneServer*>(stub))->setShouldDeleteNavRegions(b);
}

bool ZoneServerAdapter::shouldDeleteNavRegions() {
	return (static_cast<ZoneServer*>(stub))->shouldDeleteNavRegions();
}
