/*
 *	autogen/server/zone/objects/resource/ResourceSpawn.cpp generated by engine3 IDL compiler 0.60
 */
...
int ResourceSpawn::getValueOf(const String& attribute) {
	ResourceSpawnImplementation* _implementation = static_cast<ResourceSpawnImplementation*>(_getImplementationForRead());
	if (unlikely(_implementation == NULL)) {
		if (!deployed)
			throw ObjectNotDeployedException(this);
...
int ResourceSpawnAdapter::getValueOf(const String& attribute) {
...
Luna<LuaResourceSpawn>::RegType LuaResourceSpawn::Register[] = {
	...
	{ "initializeTransientMembers", &LuaResourceSpawn::initializeTransientMembers },
...
int LuaResourceSpawn::initializeTransientMembers(lua_State *L) {
	...
	if (parameterCount == 0) {
		...
		throw LuaCallbackException(L, "invalid argument count " + String::valueOf(parameterCount) + " for lua method 'ResourceSpawn:initializeTransientMembers()'");
	}
	return 0;
}
...
int LuaResourceSpawn::getValueOf(lua_State *L) {
	...
	if (lua_isnumber(L, -1)) {
		if (parameterCount == 1) {
			...
			throw LuaCallbackException(L, "invalid argument count " + String::valueOf(parameterCount) + " for lua method 'ResourceSpawn:getValueOf(integer)'");
		}
		...
		if (parameterCount == 1) {
			...
			throw LuaCallbackException(L, "invalid argument count " + String::valueOf(parameterCount) + " for lua method 'ResourceSpawn:getValueOf(string)'");
		}
	} else {
