...
ManagedWeakReference<CreatureObject* > Jukebox::getOwner() const {
	JukeboxImplementation* _implementation = static_cast<JukeboxImplementation*>(_getImplementationForRead());
	if (unlikely(_implementation == NULL)) {
		if (!deployed)
			throw ObjectNotDeployedException(this);
...
ManagedWeakReference<CreatureObject* > JukeboxImplementation::getOwner() const{
	// server/zone/objects/tangible/eventperk/Jukebox.idl():  		return owner;
	return owner;
}
...
ManagedWeakReference<CreatureObject* > JukeboxAdapter::getOwner() const {
	return (static_cast<Jukebox*>(stub))->getOwner();
}
