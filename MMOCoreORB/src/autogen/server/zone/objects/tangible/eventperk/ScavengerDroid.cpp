...
bool ScavengerDroidImplementation::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0x56f958d: //ScavengerDroid.turnedInItemList
		TypeInfo<VectorMap<unsigned long long, Vector<String>*> >::parseFromBinaryStream(&turnedInItemList, stream);
		return true;
...
int ScavengerDroidImplementation::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<VectorMap<unsigned long long, Vector<String>*> >::toBinaryStream(&turnedInItemList, stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
...
bool ScavengerDroidImplementation::hasItemInPlayerItemList(unsigned long long playerID, const String& item) {
	// server/zone/objects/tangible/eventperk/ScavengerDroid.idl():  		return 
	if (!(&turnedInItemList)->contains(playerID))	// server/zone/objects/tangible/eventperk/ScavengerDroid.idl():  			return false;
	return false;
	// server/zone/objects/tangible/eventperk/ScavengerDroid.idl():  		return turnedInItemList.get(playerID).contains(item);
	return (&turnedInItemList)->get(playerID)->contains(item);
}

int ScavengerDroidImplementation::getTurnedInItemListSize(unsigned long long playerID) {
	// server/zone/objects/tangible/eventperk/ScavengerDroid.idl():  		return 
	if (!(&turnedInItemList)->contains(playerID))	// server/zone/objects/tangible/eventperk/ScavengerDroid.idl():  			return 0;
	return 0;
	// server/zone/objects/tangible/eventperk/ScavengerDroid.idl():  		return turnedInItemList.get(playerID).size();
