...
int LuaCustomIngredient::getValueOf(lua_State *L) {
	...
	if (lua_isnumber(L, -1)) {
		if (parameterCount == 1) {
			...
			throw LuaCallbackException(L, "invalid argument count " + String::valueOf(parameterCount) + " for lua method 'CustomIngredient:getValueOf(integer)'");
		}
		...
		if (parameterCount == 1) {
			...
			throw LuaCallbackException(L, "invalid argument count " + String::valueOf(parameterCount) + " for lua method 'CustomIngredient:getValueOf(string)'");
		}
	} else {
