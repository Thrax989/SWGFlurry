/*
 *	autogen/server/zone/objects/player/sui/SuiBox.cpp generated by engine3 IDL compiler 0.60
 */
...
void SuiBoxAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_GENERATEHEADER__SUICREATEPAGEMESSAGE_:
		{
			SuiCreatePageMessage* message = static_cast<SuiCreatePageMessage*>(inv->getObjectParameter());
	...
	case RPC_GENERATECLOSEMESSAGE__:
		{
			...
			DistributedObject* _m_res = generateCloseMessage();
			resp->insertLong(_m_res == NULL ? 0 : _m_res->_getObjectID());
		}
		break;
