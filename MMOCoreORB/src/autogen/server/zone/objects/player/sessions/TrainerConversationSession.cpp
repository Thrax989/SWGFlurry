/*
 *	autogen/server/zone/objects/player/sessions/TrainerConversationSession.cpp generated by engine3 IDL compiler 0.60
 */
...
DistributedObjectServant* TrainerConversationSession::_getImplementationForRead() const {
	return _impl;
}
	...
	case RPC_ADDTRAINABLESKILL__STRING_:
		{
			...
			addTrainableSkill(skillName);
			
		}
		break;
	case RPC_GETTRAINABLESKILLS__INT_:
		{
			int selectedOption = inv->getSignedIntParameter();
			
			String _m_res = getTrainableSkills(selectedOption);
	...
	case RPC_GETTRAINABLESKILLSCOUNT__:
		{
			...
			int _m_res = getTrainableSkillsCount();
	...
	case RPC_GETNEXTSKILL__INT_:
		{
			...
			String _m_res = getNextSkill(selectedOption);
			resp->insertAscii(_m_res);
		}
		break;
	...
	case RPC_GETSELECTEDSKILL__:
		{
			...
			String _m_res = getSelectedSkill();
			resp->insertAscii(_m_res);
		}
		break;
	...
	case RPC_ADDADDITIONALMASTERSKILL__STRING_:
		{
			...
			addAdditionalMasterSkill(skillName);
	...
	case RPC_GETADDITIONALMASTERSKILL__INT_:
		{
			...
			String _m_res = getAdditionalMasterSkill(selectedOption);
			resp->insertAscii(_m_res);
		}
		break;
	case RPC_GETADDITIONALMASTERSKILLSCOUNT__:
		{
			...
			int _m_res = getAdditionalMasterSkillsCount();
			resp->insertSignedInt(_m_res);
		}
		break;
	default:
		ConversationSessionAdapter::invokeMethod(methid, inv);
	}
}
