...
void CreatureObject::updatePostureChangeDelay(int delay) {
	CreatureObjectImplementation* _implementation = static_cast<CreatureObjectImplementation*>(_getImplementation());
	if (unlikely(_implementation == NULL)) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_UPDATEPOSTURECHANGEDELAY__INT_);
		method.addSignedIntParameter(delay);
...
CreatureObject* CreatureObject::_asCreatureObject() {
	CreatureObjectImplementation* _implementation = static_cast<CreatureObjectImplementation*>(_getImplementationForRead());
	if (unlikely(_implementation == NULL)) {
		throw ObjectNotLocalException(this);
...
void CreatureObjectImplementation::destroyObjectFromDatabase(bool destroyContainedObjects) {
	// server/zone/objects/creature/CreatureObject.idl():  		clearBuffs(false);
	clearBuffs(false);
...
void CreatureObjectImplementation::updatePostureChangeDelay(int delay) {
...
void CreatureObjectAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_HEALDOT__LONG_INT_:
		{
			...
			bool _m_res = healDot(dotType, reduction);
			resp->insertBoolean(_m_res);
		}
		break;
	...
	case RPC_UPDATELASTSUCCESSFULCOMBATACTION__:
		...
		break;
	case RPC_UPDATEPOSTURECHANGEDELAY__INT_:
		{
			int delay = inv->getSignedIntParameter();
	...
	case RPC_UPDATECOOLDOWNTIMER__STRING_INT_:
		{
			...
			unsigned int miliSecondsToAdd = inv->getUnsignedIntParameter();
	...
	case RPC_ADDCOOLDOWN__STRING_INT_:
		{
			...
			unsigned int miliseconds = inv->getUnsignedIntParameter();
...
void CreatureObjectAdapter::updatePostureChangeDelay(int delay) {
