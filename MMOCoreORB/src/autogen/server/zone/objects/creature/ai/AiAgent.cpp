...
unsigned int AiAgent::getFollowState() {
	AiAgentImplementation* _implementation = static_cast<AiAgentImplementation*>(_getImplementation());
	if (unlikely(_implementation == NULL)) {
		if (!deployed)
			throw ObjectNotDeployedException(this);
...
unsigned int AiAgentImplementation::getFollowState() {
	// server/zone/objects/creature/ai/AiAgent.idl():  		}
{
	Locker _locker((&targetMutex));
...
float AiAgentImplementation::getReducedResist(float value) {
	...
	if (CreatureObjectImplementation::shockWounds <= 500){
	...
	case RPC_ISSCENTMASKED__CREATUREOBJECT_:
		{
			...
			bool _m_res = isScentMasked(target);
			resp->insertBoolean(_m_res);
		}
		break;
	case RPC_ISCONCEALED__CREATUREOBJECT_:
		{
			...
			bool _m_res = isConcealed(target);
			resp->insertBoolean(_m_res);
		}
		break;
...
unsigned int AiAgentAdapter::getFollowState() {
