RebelTravelConversationHandler = Object:new {
}

function RebelTravelConversationHandler:runScreenHandlers(pConversationTemplate, pConversingPlayer, pConversingNPC, selectedOption, pConversationScreen)
				...
				if (player:isGrouped()) then
					...
					for i = 0, gSize do
						...
						if (groupMember ~= nil) then
							...
							playerInv:switchZone("dungeon1", -20, 0, 0, 4336532)
...
function RebelTravelConversationHandler:getInitialScreen(pPlayer, pNpc, pConversationTemplate)
	local convoTemplate = LuaConversationTemplate(pConversationTemplate)
...
function RebelTravelConversationHandler:getNextConversationScreen(pConversationTemplate, pPlayer, selectedOption, pConversingNpc)
