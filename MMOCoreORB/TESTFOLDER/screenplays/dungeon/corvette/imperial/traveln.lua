NeutralTravelConversationHandler = Object:new {
}

function NeutralTravelConversationHandler:runScreenHandlers(pConversationTemplate, pConversingPlayer, pConversingNPC, selectedOption, pConversationScreen)
						...
						if (groupMember ~= nil) then
							if (groupMember ~= pConversingPlayer) then
								...
								if (groupMemberC:isImperial()) then
									...
									local suiMessage = "Do you wish to travel to the Corvette Treasure dungeon?"
							...
							playerInv:switchZone("dungeon1", -20, 0, 0, 4335862)
...
function NeutralTravelConversationHandler:getInitialScreen(pPlayer, pNpc, pConversationTemplate)
	local convoTemplate = LuaConversationTemplate(pConversationTemplate)
...
function NeutralTravelConversationHandler:getNextConversationScreen(pConversationTemplate, pPlayer, selectedOption, pConversingNpc)
