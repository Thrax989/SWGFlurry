...
function deathWatchWaterValve:handleObjectMenuSelect(pSceneObject, pPlayer, selectedID)
	...
	return ObjectManager.withCreatureObject(pPlayer, function(creature)
		...
		if name == "Water Pressure Valve A" then
			number = 1
		elseif name == "Water Pressure Valve B" then
			number = 2
		elseif name == "Water Pressure Valve C" then
			number = 3
		elseif name == "Water Pressure Valve D" then
			number = 4
		end

		DeathWatchBunkerScreenPlay:doValveSwitch(pPlayer, number)
