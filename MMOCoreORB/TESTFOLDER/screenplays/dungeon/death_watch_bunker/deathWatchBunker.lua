...
--require("/dungeon/death_watch_bunker/deathWatchSpawnMap")
...
DeathWatchBunkerScreenPlay = ScreenPlay:new {
	...
	doorData = {
		{ cellAccess = 5996316, lockTime = 1, doorType = 1 }, -- Entrance
		{ cellAccess = 5996326, lockTime = 1, doorType = 2 }, -- Area A
		{ cellAccess = 5996338, lockTime = 1, doorType = 2 }, -- Area B
		{ cellAccess = 5996352, lockTime = 1, doorType = 2 }, -- Mines
...
function DeathWatchBunkerScreenPlay:spawnObjects()
	...
	spawnedSceneObject = LuaSceneObject(spawnedPointer)
	local spawnID = spawnedSceneObject:getObjectID()
	writeData(spawnID .. ":dwb:accessEnabled", 1)
	spawnedSceneObject:setCustomObjectName("Entry Terminal")
	...
	spawnedSceneObject = LuaSceneObject(spawnedPointer)
	spawnID = spawnedSceneObject:getObjectID()
	writeData(spawnID .. ":dwb:accessEnabled", 1)
	spawnedSceneObject:setCustomObjectName("Terminal A")
	...
	spawnID = spawnedSceneObject:getObjectID()
	writeData(spawnID .. ":dwb:accessEnabled", 1)
	spawnedSceneObject:setCustomObjectName("Terminal B")
	...
	spawnID = spawnedSceneObject:getObjectID()
	writeData(spawnID .. ":dwb:accessEnabled", 1)
	spawnedSceneObject:setCustomObjectName("Terminal Mines")
	...
	spawnedSceneObject:setCustomObjectName("Water Pressure Valve A")
	...
	spawnedSceneObject:_setObject(spawnedPointer)
	spawnedSceneObject:setObjectMenuComponent("deathWatchWaterValve")
	spawnedSceneObject:setCustomObjectName("Water Pressure Valve B")
	...
	spawnedSceneObject:_setObject(spawnedPointer)
	spawnedSceneObject:setObjectMenuComponent("deathWatchWaterValve")
	spawnedSceneObject:setCustomObjectName("Water Pressure Valve C")
	...
	spawnedSceneObject:_setObject(spawnedPointer)
	spawnedSceneObject:setObjectMenuComponent("deathWatchWaterValve")
	spawnedSceneObject:setCustomObjectName("Water Pressure Valve D")
	...
	spawnedSceneObject:_setObject(spawnedPointer)
	self:setLootBoxPermissions(spawnedPointer)
	spawnedSceneObject:setCustomObjectName("Coal Bin")
	...
	spawnedSceneObject:_setObject(spawnedPointer)
	self:setLootBoxPermissions(spawnedPointer)
	spawnedSceneObject:setCustomObjectName("Armoire")
...
function DeathWatchBunkerScreenPlay:onEnterDWB(sceneObject, pCreature)
	...
	DeathWatchBunkerScreenPlay:lockAll(pCreature)
...
function DeathWatchBunkerScreenPlay:boxLooted(pSceneObject, pCreature, selectedID)
	...
	local name = SceneObject(pSceneObject):getCustomObjectName()
	if readData(name .. ":dwb:spawned") ~= 1 then
		local boxId = readData(objectID .. ":dwb:lootbox")
		writeData(name .. ":dwb:spawned", 1)
...
function DeathWatchBunkerScreenPlay:refillContainer(pSceneObject)
	...
	writeData(SceneObject(pSceneObject):getCustomObjectName() .. ":dwb:spawned", 0)
...
function DeathWatchBunkerScreenPlay:checkDoor(pSceneObject, pCreature)
	if pCreature == nil or pSceneObject == nil then
		...
		local pObject = LuaSceneObject(pSceneObject)
	ObjectManager.withCreatureObject(pCreature, function(creature)
		local doorEnabled = readData(pObject:getObjectID() .. ":dwb:accessEnabled")
		local name = pObject:getCustomObjectName()
	
		if name == "Entry Terminal" then
			doorNumber = 1
			if not creature:hasScreenPlayState(doorNumber, "death_watch_bunker") then
		...
		elseif (name == "Terminal A") or (name == "Terminal B") or (name == "Terminal Mines") then
			if name == "Terminal A" then
				doorNumber = 2
			elseif (name == "Terminal B") then
				doorNumber = 3
			else
				doorNumber = 4
			end
		...
		elseif (name == "Armorsmith Crafting Room Entry Terminal") or (name == "Droid Engineer Crafting Room Entry Terminal") or (name == "Tailor Crafting Room Entry Terminal") then
			if name == "Armorsmith Crafting Room Entry Terminal" then
				doorNumber = 5
			elseif (name == "Droid Engineer Crafting Room Entry Terminal") then
				doorNumber = 6
			else
				doorNumber = 7
			end
		...
		writeData(pObject:getObjectID() .. ":dwb:accessEnabled", 0)
