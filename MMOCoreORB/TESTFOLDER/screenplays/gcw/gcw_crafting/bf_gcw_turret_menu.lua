...
function bf_gcw_turret_menu:fillObjectMenuResponse(pSceneObject, pMenuResponse, pPlayer)
		...
		if ltoSOB:getFaction() == LuaTangibleObject(pPlayer):getFaction() then
			...
			if CreatureObject(pPlayer):hasSkill("crafting_artisan_master") == true then
				if readData((pointer)..":bf_Cloner") == 1 then
					response:addRadialMenuItem(102, 3, "Block Turret")
					...
					response:addRadialMenuItem(22, 3, "Construct Turret")
				end				
	...
	elseif mask == 4 then
		if ltoSOB:getFaction() == LuaTangibleObject(pPlayer):getFaction() then
			...
			if CreatureObject(pPlayer):hasSkill("crafting_artisan_master") then
				response:addRadialMenuItem(25, 3, "Activate Turret")
				if rank < 4 then
...
function bf_gcw_turret_menu:handleObjectMenuSelect(pSceneObject, pPlayer, selectedID)
	if not LuaSceneObject(pSceneObject):isValid() or pPlayer == nil then
		return false
	end
	...
	if (selectedID == 112) and playerObject:hasGodMode() then
		createEvent(3000,"bf_gcw_troop_menu","rename",pSceneObject,"Turret Control Point")
...
-- Un-claimed control point
	if ltoSOB:getPvpStatusBitmask() == 0 then
		if name == "Turret Control Point" or name == "Turret Control Point (Rebel)" or name == "Turret Control Point (Imperial)" then
			if ltoSOB:isNeutral() and (selectedID == 101) and obType == 0 then				
				Object:setCustomObjectName("Capturing ("..faction..")")
				createEvent(8000, "bf_gcw_turret_menu", "testa",pSceneObject)
			...
			elseif ltoSOB:getFaction() == LuaTangibleObject(pPlayer):getFaction() then
				if (selectedID == 100) then
					bf_gcw_turret_menu:checkStatus(pSceneObject)
				elseif playerCreature:hasSkill("crafting_artisan_master") then
					if (selectedID == 22) then
						...
						createEvent(11000,"bf_gcw_turret_menu","testb",pSceneObject,faction)
						createEvent(1000,"bf_gcw_turret_menu","testc",pPlayer,faction)
						ltoSOB:setPvpStatusBitmask(1)
						playerCreature:setPosture(1)
			...
			elseif (selectedID == 21) then
				--bf_gcw_turret_menu:setSabotage(pSceneObject,pPlayer,faction)
			end
		end		
-- Troop Spawner
	elseif ltoSOB:getPvpStatusBitmask() == 4 and (name == "Rebel Turret" or name == "Imperial Turret") then
		if ltoSOB:getFaction() == LuaTangibleObject(pPlayer):getFaction() then
			if (selectedID == 100) then
				bf_gcw_turret_menu:checkStatus(pSceneObject)
			elseif (selectedID == 25) then
				if ltoSOB:isRebel() then
					Object:setCustomObjectName("(Rebel Turret)")
				else
					Object:setCustomObjectName("(Imperial Turret)")
				end	
				bf_gcw_turret_menu:spawnTroop(pSceneObject)
			elseif playerCreature:hasSkill("crafting_artisan_master") then
...
--Check Status
function bf_gcw_turret_menu:checkStatus(pSceneObject)
	if LuaSceneObject(pSceneObject) ~= nil then
		local pointer = ("bf_troop:"..tostring(pSceneObject))
		local status = "Turret Status"
...
--test capture
function bf_gcw_turret_menu:testa(pSceneObject)
	local Object = LuaSceneObject(pSceneObject)
	...
	if name == "Capture Aborted" then
		Object:setCustomObjectName("Turret Control Point")
		ltObject:setPvpStatusBitmask(0)
		return 0
	elseif  name == "Capturing (Rebel)" then
		Object:setCustomObjectName("Turret Control Point (Rebel)")
		ltObject:setFaction(0x16148850)
	elseif name == "Capturing (Imperial)" then
		Object:setCustomObjectName("Turret Control Point (Imperial)")
...
--test spawner
function bf_gcw_turret_menu:testb(pSceneObject,faction)
	local Object = LuaSceneObject(pSceneObject)
	local name = Object:getCustomObjectName()
	if name == "Build Aborted ("..faction..")" then
		Object:setCustomObjectName("Turret Control Point ("..faction..")")
		LuaTangibleObject(pSceneObject):setPvpStatusBitmask(0)
	elseif name == "Building ("..faction..")" then
		Object:setCustomObjectName(""..faction.." Turret")
...
--position test
function bf_gcw_turret_menu:testc(pPlayer,faction)
	local player = LuaCreatureObject(pPlayer)
	...
	elseif name == faction.." Turret" or name == "Turret Control Point ("..faction..")" or name == "Work Complete" then
...
-- Spawn Turret
function bf_gcw_turret_menu:spawnTroop(pSceneObject)
	local sObject = LuaSceneObject(pSceneObject)
	...
	local troop = bf_gcw_turret_menu:getTroop(pSceneObject)
	local trooper = spawnSceneObject(zone,troop,sOBX,sOBZ-.5,sOBY,sOBP,math.rad(sOBD))
	local turret = LuaSceneObject(trooper)
	local name = LuaSceneObject(pSceneObject):getCustomObjectName()
	local pointer = ("bf_troop:"..tostring(pSceneObject))
	local pType = readData((pointer)..":bf_type")
	if  name == "(Rebel Turret)" then
		turret:setCustomObjectName("Rebel Turret")
		LuaTangibleObject(trooper):setFaction(0x16148850)
	elseif name == "(Imperial Turret)" then
		turret:setCustomObjectName("Imperial Turret")
		LuaTangibleObject(trooper):setFaction(0xDB4ACC54)
	end
	...
	createObserver(OBJECTDESTRUCTION, "bf_gcw_turret_menu", "troopsDead", trooper)
	LuaSceneObject(trooper):addPendingTask(3600000,"bf_gcw_turret_menu","deSpawn")
	return 0
end

-- Get troop to spawn
function bf_gcw_turret_menu:getTroop(pSceneObject)
	local pointer = ("bf_troop:"..tostring(pSceneObject))
	local rank = readData((pointer)..":bf_Rank")
	local upgradeLevel = bf_gcw_troop_menu:toAlpha(rank)
	local turret = " "
	if readData((pointer)..":bf_Cloner") == 2 then
		turret = ("object/installation/gcw_turret/gcw_block_"..upgradeLevel..".iff")
	elseif readData((pointer)..":bf_Cloner") == 3 then
		turret = ("object/installation/gcw_turret/gcw_dish_"..upgradeLevel..".iff")
	elseif readData((pointer)..":bf_Cloner") == 4 then
		turret = ("object/installation/gcw_turret/gcw_tower_"..upgradeLevel..".iff")
	end
	return turret
end

function bf_gcw_turret_menu:deSpawn(trooper)
	bf_gcw_turret_menu:troopsDead(trooper)
	bf_tools:destroy(trooper)
	return 0
end

--notify troop died
function bf_gcw_turret_menu:troopsDead(trooper,player)
	local tobID = LuaSceneObject(trooper):getObjectID()
	...
	LuaSceneObject(trooper):cancelPendingTask("bf_gcw_turret_menu","deSpawn")
	deleteData(tostring(tobID)..":bf_Capturing")
	createEvent(timer,"bf_gcw_turret_menu","resetSpawn",pSceneObject)
	return 0
end

--Reset Spawn
function bf_gcw_turret_menu:resetSpawn(pSceneObject)
	local name = LuaSceneObject(pSceneObject):getCustomObjectName()
	if name == "(Rebel Turret)" or name == "(Imperial Turret)" then
		bf_gcw_troop_menu:rename(pSceneObject, "Turret Control Point")
		LuaTangibleObject(pSceneObject):setPvpStatusBitmask(0)
	end
end

--Sabotage   ( LOOP )
function bf_gcw_turret_menu:Sabotage(pSceneObject,oldName)
	local Object = LuaSceneObject(pSceneObject)
	local name = Object:getCustomObjectName()
	local sobID = Object:getObjectID()
	local pointer = ("bf_troop:"..tostring(pSceneObject))
	...
	elseif name == "Sabotaging" then
		...
		if timer > 0 then
			...
			createEvent(5000,"bf_gcw_turret_menu","Sabotage",pSceneObject,oldName)
		else
...
--set Sabotage
function bf_gcw_turret_menu:setSabotage(pSceneObject,pPlayer,faction)
	if LuaSceneObject(pSceneObject) ~= nil and pPlayer ~= nil then
		...
		createEvent(4999,"bf_gcw_turret_menu","Sabotage",pSceneObject,name)
		createEvent(1000,"bf_gcw_turret_menu","testc",pPlayer,faction)
	end
	return 0
end
