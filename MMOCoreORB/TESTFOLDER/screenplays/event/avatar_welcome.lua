...
avatar_welcome = ScreenPlay:new {
   numberOfActs = 1, 
   questString = "avatar_welcome",
   states = {}, 
}

registerScreenPlay("avatar_welcome", true)

function avatar_welcome:start() 
   -- Spawn our character into the world, setting pLarry a pointer variable we can use to check or change his state.
   local pMerchant = spawnMobile("corellia", "salvage_bandit_leader", 1, -154.0, 28, -4719.0, 90, 0 )

end

avatar_welcome_convo_handler = Object:new {
   tstring = "conversation_exchange_dealer"
 }

function avatar_welcome_convo_handler:getNextConversationScreen(conversationTemplate, conversingPlayer, selectedOption)
...
function avatar_welcome_convo_handler:runScreenHandlers(conversationTemplate, conversingPlayer, conversingNPC, selectedOption, conversationScreen)
