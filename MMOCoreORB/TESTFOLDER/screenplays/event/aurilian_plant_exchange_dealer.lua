...
aurilian_plant_exchange = ScreenPlay:new {
   numberOfActs = 1,
   questString = "aurilian_plant_exchange_task",
   states = {},
}

registerScreenPlay("aurilian_plant_exchange", true)

function aurilian_plant_exchange:start()
   -- Spawn our character into the world, setting pLarry a pointer variable we can use to check or change his state.
   local pMerchant = spawnMobile("dathomir", "aurilian_botanist", 1, 5311, 78, -4220.9, 12, 0 )

end

aurilian_plant_exchange_convo_handler = Object:new {
   tstring = "conversation_exchange_dealer"
 }

function aurilian_plant_exchange_convo_handler:getNextConversationScreen(conversationTemplate, conversingPlayer, selectedOption)
   ...
   if ( conversation ~= nil ) then
      ...
      if ( lastConversationScreen == nil ) then
         ...
         for i = 0, containerSize - 1, 1 do
	         ...
	         local InvObj = LuaSceneObject(pInvObj)
	         	...
	         	if (InvObj:getObjectName() == "mystical_powered_orb")   then
	...
	--  Option Selections
		...
		-- Aurilian Plant
	     ...
	     elseif (optionLink == "1" and itemCounter >= 50) then
	            local pItem = giveItem(pInventory, "object/tangible/loot/plant_grow/plant_stage_1.iff", -1)
	            DeleteItems = 50
		    nextConversationScreen = conversation:getScreen("end")
		    creature:sendSystemMessage("You have received a Small Aurilian Plant.")
          ...
          for i = containerSize - 1 , 0 , -1 do
        	...
        	invObj = LuaSceneObject(pInvObj)
     	    -- Get Objectnames from Each Item and check if its the choosen Item.
		     	if (invObj:getObjectName() == "mystical_powered_orb" and DeleteItems > 0 ) then
...
function aurilian_plant_exchange_convo_handler:runScreenHandlers(conversationTemplate, conversingPlayer, conversingNPC, selectedOption, conversationScreen)
