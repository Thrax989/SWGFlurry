--Code Modified by Manik, 4.3.2017 and belongs solely to BloodfinEMU
...
local treasureMapData = {
	...
	{ planet = "tatooine", x = 3800, y = -1000 }, -- Bounty Hunter
	{ planet = "tatooine", x = 6700, y = 4240 } -- Scythe Schematic
}

local TREASURE_CHEST_LIFESPAN = 1200000 --20 minutes
...
function TreasureMapMenuComponent:spawnTreasureLoot(pChest, pPlayer, mapType)
	...
	if (mapType == 1 or mapType == 2 or mapType == 3) then
		local playerLevelRange = getRandomNumber(CreatureObject(pPlayer):getLevel() +150, CreatureObject(pPlayer):getLevel() +250)

		for i = 1, 2 do
			createLoot(pChest, "treasure_map_chest", playerLevelRange, false)
		end
	else
...
function TreasureMapMenuComponent:spawnTreasureDefenders(pObject, pPlayer, x, z, y, mapType)
	...
	if (mapType == 1 or mapType == 2) then
		...
		local pMobile = spawnMobile(mapData.planet, "pirate_overlord", 0, firstSpawnPoint[1], firstSpawnPoint[2], firstSpawnPoint[3], 0, 0)
		...
		local pMobile2 = spawnMobile(mapData.planet, "pirate_leader1", 0, secondSpawnPoint[1], secondSpawnPoint[2], secondSpawnPoint[3], 0, 0)
		TreasureMapMenuComponent:setDefenderAggro(pMobile2, pPlayer)
		local pMobile3 = spawnMobile(mapData.planet, "pirate_armsman1", 0, thirdSpawnPoint[1], thirdSpawnPoint[2], thirdSpawnPoint[3], 0, 0)
		TreasureMapMenuComponent:setDefenderAggro(pMobile3, pPlayer)
	elseif (mapType == 3) then
		local pMobile = spawnMobile(mapData.planet, "boba_fett_1", 0, firstSpawnPoint[1], firstSpawnPoint[2], firstSpawnPoint[3], 0, 0)
		spatialChat(pMobile, "@treasure_map/treasure_map:bark_" .. mapStringName[mapType])
		TreasureMapMenuComponent:setDefenderAggro(pMobile, pPlayer)
	else
